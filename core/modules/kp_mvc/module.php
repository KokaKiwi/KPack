<?php
	...
	KPack::register_module("kp_mvc", "Model View Controller System for KPack", "4.0");
	...
	class KP_MVC extends Module {
			...
			function pre_process() {
				...
				Kpack::get_class("kp_smarty", "KP_Smarty")->smarty->template_dir = KP_APPS_PATH . "/views";
			}
			
			function post_process() {
				$request = KPack::get_class("kp_url", "KP_URL")->uri;
				if(file_exists(KP_APPS_PATH . '/public/' . $request) && $request != '') {
					...
					$smarty = KPack::get_class("kp_smarty", "KP_Smarty");
					$smarty->smarty->left_delimiter = '{{';
					$smarty->smarty->right_delimiter = '}}';
					echo $smarty->smarty->fetch(KP_APPS_PATH . '/public/' . $request);
					$smarty->smarty->left_delimiter = '{';
					$smarty->smarty->right_delimiter = '}';*/
					switch($ext) {
						case "css":
						...
						case "js":
							...
							$file = file_get_contents(KP_APPS_PATH . '/public/' . $request);
							echo $file;
							break;
