...
class Smarty_Internal_Utility
{
     ...
     *            file extension
     * @param $force_compile bool
     *            force all to recompile
     * @param $time_limit int           
     * @param $max_errors int           
     * @return integer number of template files recompiled
     */
    function compileAllTemplates ($extention = '.tpl', $force_compile = false, 
            $time_limit = 0, $max_errors = null)
    {
        ...
        foreach ((array) $this->smarty->template_dir as $_dir) {
            ...
            foreach ($_compile as $_fileinfo) {
                if (strpos($_fileinfo, '.svn') !== false)
                    continue;
                $_file = $_fileinfo->getFilename();
                if (! substr_compare($_file, $extention, - strlen($extention)) ==
                         0)
                            continue;
                        if ($_fileinfo->getPath() == substr($_dir, 0, - 1)) {
                            ...
                            $_template_file = substr($_fileinfo->getPath(), 
                                    strlen($_dir)) . DS . $_file;
                        }
                        echo '<br>', $_dir, '---', $_template_file;
                        flush();
                        $_start_time = microtime(true);
                        try {
                            $_tpl = $this->smarty->createTemplate(
                                    $_template_file);
                            if ($_tpl->mustCompile()) {
                                $_tpl->compileTemplateSource();
                                echo ' compiled in  ', microtime(true) -
                                         $_start_time, ' seconds';
                                flush();
                            }
                            else {
             ...
             * @return integer number of template files recompiled
             */
            function compileAllConfig ($extention = '.conf', $force_compile = false, 
                    $time_limit = 0, $max_errors = null)
            {
                // switch off time limit
                if (function_exists('set_time_limit')) {
                ...
                foreach ((array) $this->smarty->config_dir as $_dir) {
                    ...
                    foreach ($_compile as $_fileinfo) {
                        if (strpos($_fileinfo, '.svn') !== false)
                            continue;
                        $_file = $_fileinfo->getFilename();
                        if (! substr_compare($_file, $extention, 
                                - strlen($extention)) == 0)
                            continue;
                        if ($_fileinfo->getPath() == substr($_dir, 0, - 1)) {
                            $_config_file = $_file;
                            ...
                            $_config_file = substr($_fileinfo->getPath(), 
                                    strlen($_dir)) . DS . $_file;
                        }
                        ...
                        $_start_time = microtime(true);
                        try {
                            $_config = new Smarty_Internal_Config($_config_file, 
                                    $this->smarty);
             ...
             */
            function clearCompiledTemplate ($resource_name = null, 
                    $compile_id = null, $exp_time = null)
            {
                $_compile_id = isset($compile_id) ? preg_replace('![^\w\|]+!', 
                        '_', $compile_id) : null;
                ...
                if (isset($_compile_id)) {
                    $_compile_id_part = $this->smarty->compile_dir . $_compile_id .
                             $_dir_sep;
                        }
                        $_count = 0;
                        $_compileDirs = new RecursiveDirectoryIterator($_dir);
                        ...
                        foreach ($_compile as $_file) {
                            ...
                            if ($_file->isDir()) {
                                ...
                                if ((! isset($_compile_id) ||
                                         (strlen((string) $_file) >
                                         strlen($_compile_id_part) &&
                                         substr_compare((string) $_file, 
                                                $_compile_id_part, 0, 
                                                strlen($_compile_id_part)) == 0)) &&
                                         (! isset($resource_name) ||
                                         (strlen((string) $_file) >
                                         strlen($_resource_part_1) &&
                                         substr_compare((string) $_file, 
                                                $_resource_part_1, 
                                                - strlen($_resource_part_1), 
                                                strlen($_resource_part_1)) == 0) ||
                                         (strlen((string) $_file) >
                                         strlen($_resource_part_2) &&
                                         substr_compare((string) $_file, 
                                                $_resource_part_2, 
                                                - strlen($_resource_part_2), 
                                                strlen($_resource_part_2)) == 0))) {
                                            if (isset($exp_time)) {
                                                if (time() - @filemtime($_file) >=
                                                 $exp_time) {
                                                    $_count += @unlink(
                                                            (string) $_file) ? 1 : 0;
                                        }
                    ...
                    /**
                     * Return array of tag/attributes of all tags used by an
                     * template
                     *
                     * @param $templae object
                     *            template object
