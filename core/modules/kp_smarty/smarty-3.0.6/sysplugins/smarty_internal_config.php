...
class Smarty_Internal_Config
{
    ...
    public function __construct ($config_resource, $smarty, $data = null)
    {
        ...
        if (! $this->parseConfigResourceName($config_resource)) {
            throw new SmartyException(
                    "Unable to parse config resource '{$config_resource}'");
        }
    }
    ...
    {
        return $this->config_timestamp === null ? $this->config_timestamp = filemtime(
                $this->getConfigFilepath()) : $this->config_timestamp;
    }
    ...
    private function parseConfigResourceName ($config_resource)
    {
        ...
        if (strpos($config_resource, ':') === false) {
            ...
            list ($this->config_resource_type, $this->config_resource_name) = explode(
                    ':', $config_resource, 2);
            if (strlen($this->config_resource_type) == 1) {
                ...
                $this->config_resource_type = strtolower(
                        $this->config_resource_type);
            }
        }
        return true;
    }
    ...
    public function buildConfigFilepath ()
    {
        ...
        if (file_exists($this->config_resource_name))
            ...
            // no tpl file found
        throw new SmartyException(
                "Unable to load config file \"{$this->config_resource_name}\"");
        return false;
    }
    ...
    {
        $_compile_id = isset($this->smarty->compile_id) ? preg_replace(
                '![^\w\|]+!', '_', $this->smarty->compile_id) : null;
        $_flag = (int) $this->smarty->config_read_hidden +
                 (int) $this->smarty->config_booleanize * 2 +
                 (int) $this->smarty->config_overwrite * 4;
        $_filepath = sha1($this->config_resource_name . $_flag);
        // if use_sub_dirs, break file into directories
        if ($this->smarty->use_sub_dirs) {
            $_filepath = substr($_filepath, 0, 2) . DS . substr($_filepath, 2, 
                    2) . DS . substr($_filepath, 4, 2) . DS . $_filepath;
        }
        ...
        return $_compile_dir . $_filepath . '.' .
                 basename($this->config_resource_name) . '.config' . '.php';
            }
            ...
            {
                return $this->compiled_timestamp === null ? ($this->compiled_timestamp = (file_exists(
                        $this->getCompiledFilepath())) ? filemtime(
             ...
             *
             * It does compare the timestamps of config source and the compiled
             * config and checks the force compile configuration
             *
             * @return boolean true if the file must be compiled
             */
            public function mustCompile ()
            {
                return $this->mustCompile === null ? $this->mustCompile = ($this->smarty->force_compile ||
                         $this->getCompiledTimestamp() === false ||
                         $this->smarty->compile_check &&
                         $this->getCompiledTimestamp() < $this->getTimestamp()) : $this->mustCompile;
            }

            /**
             * Returns the compiled config file
             *
             * It checks if the config file must be compiled or just read the
             * compiled version
             *
            ...
            public function getCompiledConfig ()
            {
                if ($this->compiled_config === null) {
                    ...
                    if ($this->mustCompile()) {
                        ...
                        $this->compiled_config = file_get_contents(
            ...
            public function compileConfigSource ()
            {
                ...
                Smarty_Internal_Write_File::writeFile(
                        $this->getCompiledFilepath(), $this->getCompiledConfig(), 
                        $this->smarty);
            }
            
            /*
             * load config variables @param mixed $sections array of section
             * names, single section or null @param object $scope global,parent
             * or local
             */
            public function loadConfigVars ($sections = null, $scope = 'local')
            {
                if ($this->data instanceof Smarty_Internal_Template) {
                    $this->data->properties['file_dependency'][sha1(
                            $this->getConfigFilepath())] = array(
                            $this->getConfigFilepath(), $this->getTimestamp(), 
                            'file');
                }
                if ($this->mustCompile()) {
                    $this->compileConfigSource();
                ...
                foreach ($_config_vars['vars'] as $variable => $value) {
                    if ($this->smarty->config_overwrite ||
                             ! isset($scope_ptr->config_vars[$variable])) {
                                $scope_ptr->config_vars[$variable] = $value;
                            }
                            else {
                                $scope_ptr->config_vars[$variable] = array_merge(
                                        (array) $scope_ptr->config_vars[$variable], 
                                        (array) $value);
                            }
                        }
                        // scan sections
                        foreach ($_config_vars['sections'] as $this_section => $dummy) {
                            if ($sections == null ||
                                     in_array($this_section, (array) $sections)) {
