...
class Smarty_Internal_Compile_Insert extends Smarty_Internal_CompileBase
{
    ...
    public function compile ($args, $compiler)
    {
        ...
        if (isset($_attr['assign'])) {
            // output will be stored in a smarty variable instead of being
            // displayed
            $_assign = $_attr['assign'];
            // create variable to make shure that the compiler knows about its
            // nocache status
            $this->compiler->template->tpl_vars[trim($_attr['assign'], "'")] = new Smarty_Variable(
                    null, true);
        }
                     ...
                     file_exists($_script)) {
                        ...
                        if (! empty($_dir)) {
                            foreach ((array) $_dir as $_script_dir) {
                                if (strpos('/\\', substr($_script_dir, - 1)) ===
                                         false) {
                                            $_script_dir .= DS;
                                        }
                            ...
                            if ($_filepath == false) {
                                $this->compiler->trigger_template_error(
                                        "{insert} missing script file '{$_script}'", 
                            ...
                            if (! is_callable($_function)) {
                                // try plugin
                                if (! $_function = $this->compiler->getPlugin(
                                        $_name, 'insert')) {
                                    $this->compiler->trigger_template_error(
