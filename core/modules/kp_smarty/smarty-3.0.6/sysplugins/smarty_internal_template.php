...
class Smarty_Internal_Template extends Smarty_Internal_Data
{
    ...
    // special properties
    public $properties = array('file_dependency' => array(), 'nocache_hash' => '', 
            'function' => array());
    // required plugins
     ...
     *
     * @param $template_resource string
     *            template resource string
     * @param $_parent object
     *            back pointer to parent object with variables or null
     * @param $_cache_id mixed
     *            cache id or null
     * @param $_compile_id mixed
     *            compile id or null
     */
    public function __construct ($template_resource, $smarty, $_parent = null, 
            $_cache_id = null, $_compile_id = null, $_caching = null, 
    ...
    {
        return $this->template_filepath === null ? $this->template_filepath = $this->resource_object->getTemplateFilepath(
                $this) : $this->template_filepath;
    }
        ...
        if ($this->template_source === null) {
            if (! $this->resource_object->getTemplateSource($this)) {
                throw new SmartyException(
                        "Unable to read template {$this->resource_type} '{$this->resource_name}'");
            }
        }
    ...
    public function isExisting ($error = false)
    {
        ...
        if (! $this->isExisting && $error) {
            throw new SmartyException(
                    "Unable to load template {$this->resource_type} '{$this->resource_name}'");
        }
     ...
     *
     * It does compare the timestamps of template source and the compiled
     * templates and checks the force compile configuration
     *
    ...
    public function mustCompile ()
    {
        ...
        if ($this->mustCompile === null) {
            $this->mustCompile = ($this->resource_object->usesCompiler &&
                     ($this->smarty->force_compile ||
                     $this->resource_object->isEvaluated ||
                     $this->getCompiledTimestamp() === false ||
                    // ($this->smarty->compile_check &&
                    // $this->getCompiledTimestamp () !==
                    // $this->getTemplateTimestamp ())));
                    ($this->smarty->compile_check &&
                     $this->getCompiledTimestamp() <
                     $this->getTemplateTimestamp())));
        }
    ...
    {
        return $this->compiled_filepath === null ? ($this->compiled_filepath = ! $this->resource_object->isEvaluated ? $this->resource_object->getCompiledFilepath(
                $this) : false) : $this->compiled_filepath;
    }
    ...
    public function getCompiledTimestamp ()
    {
        return $this->compiled_timestamp === null ? ($this->compiled_timestamp = (! $this->resource_object->isEvaluated &&
                 file_exists($this->getCompiledFilepath())) ? filemtime(
                        $this->getCompiledFilepath()) : false) : $this->compiled_timestamp;
            }

            /**
             * Returns the compiled template
             *
             * It checks if the template must be compiled or just read from the
             * template resource
             *
            ...
            public function getCompiledTemplate ()
            {
                if ($this->compiled_template === null) {
                    ...
                    if ($this->mustCompile()) {
                        ...
                        if ($this->compiled_template === null) {
                            $this->compiled_template = ! $this->resource_object->isEvaluated &&
                                     $this->resource_object->usesCompiler ? file_get_contents(
                                            $this->getCompiledFilepath()) : false;
                                }
                            }
                        }
                     ...
                     *
                     * If the template is not evaluated the compiled template is
                     * saved on disk
                     */
                    public function compileTemplateSource ()
                    {
                        if (! $this->resource_object->isEvaluated) {
                            ...
                            $this->properties['file_dependency'][$this->templateUid] = array(
                        ...
                        if (! is_object($this->compiler_object)) {
                            // load compiler
                            $this->smarty->loadPlugin(
                                    $this->resource_object->compiler_class);
                            $this->compiler_object = new $this->resource_object->compiler_class(
                                    $this->resource_object->template_lexer_class, 
                                    $this->resource_object->template_parser_class, 
                                    $this->smarty);
                        }
                        // compile locking
                        if ($this->smarty->compile_locking &&
                                ...
                                // call compiler
                                try {
                                    $this->compiler_object->compileTemplate(
                                            $this);
                                }
                                catch (Exception $e) {
                                ...
                                if (! $this->resource_object->isEvaluated &&
                                    ...
                                    /**
                                     * Returns the filepath of the cached
                                     * template output
                                     *
                                     * The filepath is determined by the actual
                                     * cache resource
                                     *
                                    ...
                                    public function getCachedFilepath ()
                                    {
                                        return $this->cached_filepath === null ? $this->cached_filepath = ($this->resource_object->isEvaluated ||
                                                 ...
                                                 $this->caching ==
                                                 Smarty::CACHING_LIFETIME_SAVED)) ? false : $this->cache_resource_object->getCachedFilepath(
                                                        $this) : $this->cached_filepath;
                                    }

                                    /**
                                     * Returns the timpestamp of the cached
                                     * template output
                                     *
                                     * The timestamp is determined by the actual
                                     * cache resource
                                     *
                                     * @return integer the template timestamp
                                     */
                                    public function getCachedTimestamp ()
                                    {
                                        return $this->cached_timestamp === null ? $this->cached_timestamp = ($this->resource_object->isEvaluated ||
                                                 ...
                                                 Smarty::CACHING_LIFETIME_CURRENT ||
                                                 $this->caching ==
                                                 Smarty::CACHING_LIFETIME_SAVED)) ? false : $this->cache_resource_object->getCachedTimestamp(
                                                        $this) : $this->cached_timestamp;
                                    }

                                    /**
                                     * Returns the cached template output
                                     *
                                     * @return string |booelan the template
                                     *         content or false if the file does
                                     *         not exist
                                     */
                                    public function getCachedContent ()
                                    {
                                        return $this->rendered_content === null ? $this->rendered_content = ($this->resource_object->isEvaluated ||
                                                 ...
                                                 Smarty::CACHING_LIFETIME_CURRENT ||
                                                 $this->caching ==
                                                 Smarty::CACHING_LIFETIME_SAVED)) ? false : $this->cache_resource_object->getCachedContents(
                                                        $this) : $this->rendered_content;
                                    }
                                    ...
                                    public function writeCachedContent ($content)
                                    {
                                        ...
                                        return $this->cache_resource_object->writeCachedContent(
                                                $this, 
                                                $this->createPropertyHeader(
                                                        true) . $content);
                                    }

                                    /**
                                     * Checks of a valid version redered HTML
                                     * output is in the cache
                                     *
                                     * If the cache is valid the contents is
                                     * stored in the template object
                                     *
                                        ...
                                        if ($this->isCached === null) {
                                            ...
                                            if (($this->caching ==
                                                ...
                                                if ($this->caching ===
                                                    ...
                                                    $this->rendered_content = $this->cache_resource_object->getCachedContents(
                                                            $this, $no_render);
                                                    if ($this->smarty->debugging) {
                                                    ...
                                                    if ($this->caching ===
                                                             Smarty::CACHING_LIFETIME_SAVED &&
                                                             $this->properties['cache_lifetime'] >=
                                                             0 &&
                                                             (time() >
                                                             ($this->getCachedTimestamp() +
                                                             $this->properties['cache_lifetime']))) {
                                                                $this->tpl_vars = array();
                                                                ...
                                                                foreach ($this->properties['file_dependency'] as $_file_to_check) {
                                                                    If ($_file_to_check[2] ==
                                                                        ...
                                                                        $this->getResourceTypeName(
                                                                                ...
                                                                                $resource_type, 
                                                                                $resource_name);
                                                                        $resource_handler = $this->loadTemplateResourceHandler(
                                                                                $resource_type);
                                                                        $mtime = $resource_handler->getTemplateTimestampTypeName(
                                                                                $resource_type, 
                                                                                $resource_name);
                                                                    }
                                                                    // If
                                                    ...
                                                    /**
                                                     * Render the output using
                                                     * the compiled template or
                                                     * the PHP template source
                                                     *
                                                            ...
                                                            if ($this->resource_object->isEvaluated) {
                                                                ...
                                                                if ($this->smarty->compile_check) {
                                                                    ...
                                                                    if (is_callable(
                                                                        ...
                                                                        ob_start();
                                                                        $this->resource_object->renderUncompiled(
                                                                                $this);
                                                                    }
                                                                    else {
                                                                        throw new SmartyException(
                                                                                "Resource '$this->resource_type' must have 'renderUncompiled' methode");
                                                                    }
                                                                }
                                                                $this->rendered_content = ob_get_clean();
                                                                if (! $this->resource_object->isEvaluated &&
                                                                         empty(
                                                                                $this->properties['file_dependency'][$this->templateUid])) {
                                                                            $this->properties['file_dependency'][$this->templateUid] = array(
                                                                                    $this->getTemplateFilepath(), 
                                                                                    $this->getTemplateTimestamp(), 
                                                                                    $this->resource_type);
                                                                        ...
                                                                        if ($this->parent instanceof Smarty_Internal_Template) {
                                                                            $this->parent->properties['file_dependency'] = array_merge(
                                                                                    $this->parent->properties['file_dependency'], 
                                                                                    $this->properties['file_dependency']);
                                                                        ...
                                                                        if (! $this->resource_object->isEvaluated &&
                                                                            ...
                                                                            $cache_split = preg_split(
                                                                                    "!/\*%%SmartyNocache:{$this->properties['nocache_hash']}%%\*\/(.+?)/\*/%%SmartyNocache:{$this->properties['nocache_hash']}%%\*/!s", 
                                                                            ...
                                                                            if (isset(
                                                                                    $this->smarty->autoload_filters['output']) ||
                                                                                     isset(
                                                                                        ...
                                                                                        $output = Smarty_Internal_Filter_Handler::runFilter(
                                                                                                'output', 
                                                                                                $output, 
                                                                                                $this);
                                                                                    }
                                                                            ...
                                                                            // var_dump('renderTemplate',
                                                                            // $this->has_nocache_code,
                                                                            // $this->template_resource,
                                                                            // $this->properties['nocache_hash'],
                                                                            // $this->parent->properties['nocache_hash'],
                                                                            // $this->rendered_content);
                                                                            if ($this->has_nocache_code &&
                                                                                     ! empty(
                                                                                            $this->properties['nocache_hash']) &&
                                                                                     ! empty(
                                                                                            $this->parent->properties['nocache_hash'])) {
                                                                                        // replace
                                                                                        // nocache_hash
                                                                                        $this->rendered_content = preg_replace(
                                                                                                "/{$this->properties['nocache_hash']}/", 
                                                                                                $this->parent->properties['nocache_hash'], 
                                                                                                $this->rendered_content);
                                                                    ...
                                                                    public function getRenderedTemplate ()
                                                                    {
                                                                        ...
                                                                        if ($this->rendered_content ===
                                                                                 null) {
                                                                                     ...
                                                                                     *            resource
                                                                                     *            name
                                                                                     * @param $resource_handler object
                                                                                     *            return
                                                                                     *            resource
                                                                                     *            handler
                                                                                     *            object
                                                                                     */
                                                                                    public function parseResourceName (
                                                                                            $template_resource, 
                                                                                            &$resource_type, 
                                                                                            &$resource_name, 
                                                                                            &$resource_handler)
                                                                                    {
                                                                                        ...
                                                                                        if ($resource_type !=
                                                                                                 'eval') {
                                                                                                    $this->smarty->template_objects[sha1(
                                                                                                            $this->template_resource .
                                                                                                             $this->cache_id .
                                                                                                             $this->compile_id)] = $this;
                                                                                        }
                                                                                        return true;
                                                                                    }
                                                                                                ...
                                                                                                foreach ((array) $this->smarty->template_dir as $_template_dir) {
                                                                                                    if (strpos(
                                                                                                            ...
                                                                                                            $_filepath = $_template_dir .
                                                                                                                ...
                                                                                                                // found
                                                                                                                if (! empty(
                                                                                                                        $this->smarty->default_template_handler_func)) {
                                                                                                                    if (! is_callable(
                                                                                                                            $this->smarty->default_template_handler_func)) {
                                                                                                                        throw new SmartyException(
                                                                                                                     ...
                                                                                                                     * scopes
                                                                                                                     */
                                                                                                                    public function updateParentVariables (
                                                                                                                                ...
                                                                                                                                if (strpos(
                                                                                                                                        ...
                                                                                                                                        ':') ===
                                                                                                                                         false) {
                                                                                                                                            ...
                                                                                                                                            // from
                                                                                                                                            // path
                                                                                                                                            list ($resource_type, $resource_name) = explode(
                                                                                                                                                    ':', 
                                                                                                                                                    $template_resource, 
                                                                                                                                                    2);
                                                                                                                                            if (strlen(
                                                                                                                                             ...
                                                                                                                                             *         object
                                                                                                                                             */
                                                                                                                                            protected function loadTemplateResourceHandler (
                                                                                                                                                    $resource_type)
                                                                                                                                            {
                                                                                                                                                // try
                                                                                                                                                // registered
                                                                                                                                                // resource
                                                                                                                                                if (isset(
                                                                                                                                                        $this->smarty->registered_resources[$resource_type])) {
                                                                                                                                                    ...
                                                                                                                                                    if (in_array(
                                                                                                                                                        ...
                                                                                                                                                        $_resource_class = 'Smarty_Internal_Resource_' .
                                                                                                                                                                ...
                                                                                                                                                                // plugins
                                                                                                                                                                // dir
                                                                                                                                                                $_resource_class = 'Smarty_Resource_' .
                                                                                                                                                                         ucfirst(
                                                                                                                                                                                $resource_type);
                                                                                                                                                                        if ($this->smarty->loadPlugin(
                                                                                                                                                                                $_resource_class)) {
                                                                                                                                                                            if (class_exists(
                                                                                                                                                                                    $_resource_class, 
                                                                                                                                                                                    false)) {
                                                                                                                                                                            ...
                                                                                                                                                                            if (in_array(
                                                                                                                                                                                ...
                                                                                                                                                                                if (is_object(
                                                                                                                                                                                    ...
                                                                                                                                                                                    $this->smarty->security_policy->isTrustedStream(
                                                                                                                                                                                            $resource_type);
                                                                                                                                                                                }
                                                                                                                                                                                return new Smarty_Internal_Resource_Stream(
                                                                                                                                                                                        $this->smarty);
                                                                                                                                                                            }
                                                                                                                                                                            else {
                                                                                                                                                                                throw new SmartyException(
                                                                                                                                                            ...
                                                                                                                                                            $cache = false)
                                                                                                                                                    {
                                                                                                                                                        ...
                                                                                                                                                        if (! $cache) {
                                                                                                                                                            if (! empty(
                                                                                                                                                                    $this->required_plugins['compiled'])) {
                                                                                                                                                                $plugins_string = '<?php ';
                                                                                                                                                        ...
                                                                                                                                                        if ($cache) {
                                                                                                                                                            ...
                                                                                                                                                            if (! empty(
                                                                                                                                                                    $this->smarty->template_functions)) {
                                                                                                                                                                // copy
                                                                                                                                                                // code
                                                                                                                                                                // of
                                                                                                                                                                // {function}
                                                                                                                                                                ...
                                                                                                                                                                foreach ($this->smarty->template_functions as $name => $function_data) {
                                                                                                                                                                    if (isset(
                                                                                                                                                                            $function_data['called_nocache'])) {
                                                                                                                                                                        unset(
                                                                                                                                                                                $function_data['called_nocache'], 
                                                                                                                                                                                $this->smarty->template_functions[$name]['called_nocache']);
                                                                                                                                                                        $this->properties['function'][$name] = $function_data;
                                                                                                                                                                    }
                                                                                                                                                                }
                                                                                                                                                            }
                                                                                                                                                        }
                                                                                                                                                        $properties_string .= "\$_smarty_tpl->decodeProperties(" .
                                                                                                                                                                 var_export(
                                                                                                                                                                        $this->properties, 
                                                                                                                                                                        true) .
                                                                                                                                                                 "); /*/%%SmartyHeaderCode%%*/?>\n";
                                                                                                                                                        return $properties_string .
                                                                                                                                                                    ...
                                                                                                                                                                    $properties)
                                                                                                                                                            {
                                                                                                                                                                ...
                                                                                                                                                                if (! empty(
                                                                                                                                                                        $properties['function'])) {
                                                                                                                                                                    $this->properties['function'] = array_merge(
                                                                                                                                                                            $this->properties['function'], 
                                                                                                                                                                            $properties['function']);
                                                                                                                                                                    $this->smarty->template_functions = array_merge(
                                                                                                                                                                            $this->smarty->template_functions, 
                                                                                                                                                                            $properties['function']);
                                                                                                                                                                }
                                                                                                                                                            }
                                                                                                                                                             ...
                                                                                                                                                             * assignments
                                                                                                                                                             */
                                                                                                                                                            public function createLocalArrayVariable (
                                                                                                                                                                    $tpl_var, 
                                                                                                                                                                    $nocache = false, 
                                                                                                                                                                    $scope = Smarty::SCOPE_LOCAL)
                                                                                                                                                            {
                                                                                                                                                                                                    ...
                                                                                                                                                                                                    $property_name)
                                                                                                                                                                                            {
                                                                                                                                                                                                if ($property_name ==
                                                                                                                                                                                                         'resource_object') {
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            if (! $this->parseResourceName(
                                                                                                                                                                                                                    $this->template_resource, 
                                                                                                                                                                                                                    $this->resource_type, 
                                                                                                                                                                                                                    $this->resource_name, 
                                                                                                                                                                                                                    $this->resource_object)) {
                                                                                                                                                                                                                throw new SmartyException(
                                                                                                                                                                                                                        "Unable to parse resource name \"{$template_resource}\"");
                                                                                                                                                                                                            }
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        if ($property_name ==
                                                                                                                                                                                                             ...
                                                                                                                                                                                                             * name
                                                                                                                                                                                                             * format:
                                                                                                                                                                                                             * Smarty_Method_MethodName
                                                                                                                                                                                                             * plugin
                                                                                                                                                                                                             * filename
                                                                                                                                                                                                             * format:
                                                                                                                                                                                                             * method.methodname.php
                                                                                                                                                                                                             *
                                                                                                                                                                                                             * @param $name string
                                                                                                                                                                                                             *            unknown
                                                                                                                                                                                                             *            methode
