...
class Smarty_Internal_CompileBase
{
     ...
     *
     * The attributes passed for the tag to compile are checked against the list
     * of required and
     * optional attributes. Required attributes must be present. Optional
     * attributes are check against
     * against the corresponding list. The keyword '_any' specifies that any
        ...
        foreach ($attributes as $key => $mixed) {
            ...
            if (! is_array($mixed)) {
                ...
                if (in_array($kv['key'], $this->option_flags)) {
                    if (is_bool($kv['value'])) {
                        ...
                        if (is_string($kv['value']) &&
                                 in_array(trim($kv['value'], '\'"'), 
                                    ...
                                    if (is_numeric($kv['value']) &&
                                             in_array($kv['value'], array(0, 1))) {
                                                if ($kv['value'] == 1) {
                                                    $_indexed_attr[$kv['key']] = true;
                                                }
                                                else {
                                                ...
                                                $this->compiler->trigger_template_error(
                                                        "illegal value of option flag \"{$kv['key']}\"", 
                                                        $this->compiler->lex->taglineno);
                                            }
                                    ...
                                    reset($mixed);
                                    $_indexed_attr[key($mixed)] = $mixed[key(
                                            $mixed)];
                                }
                            }
                        }
                        ...
                        if ($this->optional_attributes != array('_any')) {
                            $tmp_array = array_merge($this->required_attributes, 
                     ...
                     * @param $data anytype
                     *            optional data which shall be saved on stack
                     */
                    function _open_tag ($open_tag, $data = null)
                    {
                        array_push($this->compiler->_tag_stack, 
                                array($open_tag, $data));
                    }
                     ...
                     * expected opening tags
                     *
                     * @param $ array
                     *            |string $expected_tag the expected opening tag
                     *            names
                    ...
                    function _close_tag ($expected_tag)
                    {
                        if (count($this->compiler->_tag_stack) > 0) {
                            // get stacked info
                            list ($_open_tag, $_data) = array_pop(
                                    $this->compiler->_tag_stack);
