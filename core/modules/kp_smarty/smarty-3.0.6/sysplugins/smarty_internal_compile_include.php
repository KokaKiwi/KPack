...
class Smarty_Internal_Compile_Include extends Smarty_Internal_CompileBase
{
    ...
    public function compile ($args, $compiler)
    {
        ...
        if ($compiler->smarty->merge_compiled_includes ||
                 $_attr['inline'] === true) {
                    // check if compiled code can be merged (contains no
                    // variable part)
                    if (! $compiler->has_variable_string &&
                     (substr_count($include_file, '"') == 2 or
                     substr_count($include_file, "'") == 2) and
                     substr_count($include_file, '(') == 0) {
                        $tmp = null;
                eval("\$tmp = $include_file;");
                if ($this->compiler->template->template_resource != $tmp) {
                    $tpl = new $compiler->smarty->template_class($tmp, 
        ...
        if (isset($_attr['assign'])) {
            // output will be stored in a smarty variable instead of beind
            // displayed
        ...
        if ($this->compiler->template->caching && ! $this->compiler->nocache &&
                ...
                /*
                 * if the {include} tag provides individual parameter for
                 * caching it will not be included into the common cache file
                 * and treated like a nocache section
                 */
                ...
                unset($_attr['file'], $_attr['assign'], $_attr['cache_id'], 
                        $_attr['compile_id'], $_attr['cache_lifetime'], 
                        $_attr['nocache'], $_attr['caching'], $_attr['scope'], 
                        $_attr['inline']);
                ...
                if (isset($_assign)) {
                    ...
                    if ($has_compiled_template &&
                        ...
                        $_output .= "<?php /*  End of included template \"" .
                                 $tpl->getTemplateFilepath() . "\" */ ?>\n";
