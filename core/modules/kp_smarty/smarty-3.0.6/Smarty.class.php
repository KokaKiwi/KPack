 ...
 * Sets SMARTY_SYSPLUGINS_DIR only if user application has not already defined
 * it.
 */
...
if (SMARTY_SPL_AUTOLOAD &&
         set_include_path(
                get_include_path() . PATH_SEPARATOR . SMARTY_SYSPLUGINS_DIR) !==
         false) {
...
class Smarty extends Smarty_Internal_Data
{
    ...
    // config var settings
    public $config_overwrite = true; // Controls whether variables with the same
                                     // name overwrite each other.
    public $config_booleanize = true; // Controls whether config values of
                                      // on/true/yes and off/false/no get
                                      // converted to boolean
    public $config_read_hidden = true; // Controls whether hidden config
                                       // sections/vars are read from the file.
                                       // config vars
    ...
    public function __construct ()
    {
        ...
        if (! $this->debugging && $this->debugging_ctrl == 'URL') {
            ...
            if (false !== strpos($_query_string, $this->smarty_debug_id)) {
                if (false !==
                         strpos($_query_string, $this->smarty_debug_id . '=on')) {
                            // enable debugging for this browser session
                            setcookie('SMARTY_DEBUG', true);
                            $this->debugging = true;
                        }
                        elseif (false !==
                                 strpos($_query_string, 
                                        $this->smarty_debug_id . '=off')) {
                                    // disable debugging for this browser
                                    // session
                     ...
                     *            the resource handle of the template file or
                     *            template object
                     * @param $cache_id mixed
                     *            cache id to be used with this template
                     * @param $compile_id mixed
                     *            compile id to be used with this template
                     * @param $ object
                     *            |null $parent next higher level of Smarty
                     *            variables
                     * @return string rendered template output
                     */
                    public function fetch ($template, $cache_id = null, 
                            $compile_id = null, $parent = null, $display = false)
                    {
                        ...
                        ($template instanceof $this->template_class) ? $_template = $template : $_template = $this->createTemplate(
                                $template, $cache_id, $compile_id, $parent);
                        if (isset($this->error_reporting)) {
                            $_smarty_old_error_level = error_reporting(
                                    $this->error_reporting);
                        }
                        // obtain data for cache modified check
                        if ($this->cache_modified_check && $this->caching &&
                                 $display) {
                                    $_isCached = $_template->isCached() &&
                                     ! $_template->has_nocache_code;
                            if ($_isCached) {
                                $_gmt_mtime = gmdate('D, d M Y H:i:s', 
                                        $_template->getCachedTimestamp()) . ' GMT';
                            }
                            else {
                                $_gmt_mtime = '';
                            }
                        ...
                        if ((! $this->caching ||
                                 $_template->resource_object->isEvaluated) &&
                                 (isset($this->autoload_filters['output']) ||
                                 isset($this->registered_filters['output']))) {
                                    $_output = Smarty_Internal_Filter_Handler::runFilter(
                                            'output', 
                                            $_template->getRenderedTemplate(), 
                                            $_template);
                        }
                        else {
                     ...
                     *
                     * @param $ string
                     *            |object $template the resource handle of the
                     *            template file or template object
                     * @param $cache_id mixed
                     *            cache id to be used with this template
                     * @param $compile_id mixed
                     *            compile id to be used with this template
                    ...
                    {
                        // display template
                        $this->fetch($template, $cache_id, $compile_id, $parent, 
                                true);
                    }
                     ...
                     * @param $compile_id mixed
                     *            compile id to be used with this template
                     * @return s object template object
                     */
                    public function createTemplate ($template, $cache_id = null, 
                            $compile_id = null, $parent = null)
                    {
                        if (! empty($cache_id) &&
                                 (is_object($cache_id) || is_array($cache_id))) {
                                    $parent = $cache_id;
                                    $cache_id = null;
                                }
                                ...
                                if (! is_object($template)) {
                                    ...
                                    if (isset(
                                            $this->template_objects[$_templateId]) &&
                                             $this->caching) {
                                            ...
                                            // just return a copy of template
                                            // class
                                            $tpl = $template;
                                        }
                                        ...
                                        if (! empty($data) && is_array($data)) {
                                            ...
                                            foreach ($data as $_key => $_val) {
                                                $tpl->tpl_vars[$_key] = new Smarty_variable(
                                                        $_val);
                                            }
                                        }
                                        return $tpl;
                                     ...
                                     * @return string variable value or or array
                                     *         of variables
                                     */
                                            ...
                                            $type = null)
                                    {
                                        ...
                                        return $this->loadCacheResource($type)->clearAll(
                                                $exp_time);
                                    }
                                    ...
                                    function clearCache ($template_name, 
                                            ...
                                            $exp_time = null, $type = null)
                                    {
                                        // load cache resource and call clear
                                        return $this->loadCacheResource($type)->clear(
                                                $template_name, $cache_id, 
                                                $compile_id, $exp_time);
                                    }
                                            ...
                                            $security_class = null)
                                    {
                                        ...
                                        if (class_exists($security_class)) {
                                            $this->security_policy = new $security_class(
                                                    $this);
                                        }
                                        else {
                                     ...
                                     */
                                    public function loadCacheResource (
                                            $type = null)
                                    {
                                        if (! isset($type)) {
                                        ...
                                        if (in_array($type, 
                                                $this->cache_resource_types)) {
                                            $cache_resource_class = 'Smarty_Internal_CacheResource_' .
                                                     ucfirst($type);
                                                    return new $cache_resource_class(
                                                            $this);
                                                }
                                                else {
                                                    // try plugins dir
                                                    $cache_resource_class = 'Smarty_CacheResource_' .
                                                             ucfirst($type);
                                                            if ($this->loadPlugin(
                                                    ...
                                                    public function addTemplateDir (
                                                            $template_dir)
                                                    {
                                                        $this->template_dir = array_unique(
                                                                array_merge(
                                                     ...
                                                     *
                                                     */
                                                    function addPluginsDir (
                                                            $plugins_dir)
                                                    {
                                                        $this->plugins_dir = array_unique(
                                                                array_merge(
                                                            ...
                                                            $name)
                                                    {
                                                        ...
                                                        if (! is_object(
                                                                $this->registered_objects[$name][0]))
                                                            throw new SmartyException(
                                                    ...
                                                    /**
                                                     * Takes unknown classes and
                                                     * loads plugin files for
                                                     * them
                                                     * class name format:
                                                     * Smarty_PluginType_PluginName
                                                     * plugin filename format:
                                                     * plugintype.pluginname.php
                                                     *
                                                                            ...
                                                                            if ($first3 ==
                                                                                ...
                                                                                if (strpos(
                                                                                        ...
                                                                                        foreach (array(
                                                                                            ...
                                                                                            if (method_exists(
                                                                                                    "Smarty_Internal_{$external}", 
                                                                                                    $name)) {
                                                                                                if (! isset(
                                                                                                        $this->$external)) {
                                                                                                    $class = "Smarty_Internal_{$external}";
                                                                                                    $this->$external = new $class(
                                                                                                            $this);
                                                                                                }
                                                                                        ...
                                                                                        if (in_array(
                                                                                            ...
                                                                                            if (! isset(
                                                                                                    $this->utility)) {
                                                                                                $this->utility = new Smarty_Internal_Utility(
                                                                                                        $this);
                                                                                            }
                                                                                            return call_user_func_array(
                                                                                                    array(
                                                                                        ...
                                                                                        if (strtolower(
                                                                                                $name) ==
                                                                                                 'smarty') {
                                                                                                    throw new SmartyException(
                                                                                                            'Please use parent::__construct() to call parent constuctor');
                                                                                                    return false;
                                                                                                }
                                                                                                throw new SmartyException(
                                                                                                        "Call of unknown function '$name'.");
                                                                                            }
                                                                                        }
